...
function setupActiveNav(){
  ...
  // This avoids cases where pages live under /tentang/ but are actually in "Tentang" dropdown.
  ...
  // If you have pages under /tentang/ that are NOT listed in the menu dropdown, you can enable this:
  // if (currentPath.includes('/tentang/')) setBtnActive('#dd-profil-btn');
...
/*dot9_outline_fix_v2*/
;(()=>{const m=()=>matchMedia('(max-width:768px)').matches;const a=()=>{if(!m())return;const b=document.querySelector('.iconbtn.appsbtn,.appsbtn');if(!b)return;try{b.style.setProperty('background','transparent','important');b.style.setProperty('background-color','transparent','important');b.style.setProperty('box-shadow','none','important');b.style.setProperty('border','1.5px solid rgba(255,255,255,.65)','important');b.style.setProperty('outline','none','important');b.querySelectorAll('*').forEach(e=>{e.style.setProperty('background','transparent','important');e.style.setProperty('background-color','transparent','important');e.style.setProperty('box-shadow','none','important');});}catch(e){};};const r=()=>{let i=0;const id=setInterval(()=>{a();i++;if(document.querySelector('.iconbtn.appsbtn,.appsbtn')||i>40)clearInterval(id);},250);};document.addEventListener('DOMContentLoaded',r);window.addEventListener('resize',a);})();
...
(function(){
  ...
  function normPath(p){
    let v = (p || location.pathname || "/").replace(/index\.html$/,"");
    if (!v.startsWith("/")) v = "/" + v;
    if (v.length > 1 && !v.endsWith("/")) v += "/";
    return v;
  }
  ...
  function setActiveLinks(root){
    ...
    document.querySelectorAll(".dropdown").forEach((dd) => {
      if (dd.querySelector(".menu a.is-active")) {
        dd.querySelector(".dropbtn")?.classList.add("is-active");
      }
    });

    document.querySelectorAll(".drawer-acc").forEach((acc) => {
      const hasActive = !!acc.querySelector(".drawer-acc-panel a.is-active");
      if (hasActive) {
        acc.classList.add("open");
        acc.querySelector(".drawer-acc-btn")?.classList.add("is-active");
      }
    });
  }
  ...
  function setAutoBreadcrumb(){
    ...
    const el = document.querySelector(".page .breadcrumb");
    if (!el || !trail) return;
    const html = trail.map((item, i) => {
      const label = item[0], href = item[1], last = i === trail.length - 1;
  ...
  function bootNavEnhancements(){
    if (!document.querySelector(".navlinks") || !document.querySelector(".drawer-links")) return false;
  ...
  const timer = setInterval(() => {
    tries += 1;
    if (bootNavEnhancements() || tries > 40) clearInterval(timer);
  }, 150);

  document.addEventListener("DOMContentLoaded", bootNavEnhancements);
  window.addEventListener("load", bootNavEnhancements);
})();
